...
const required = [
  nameEl, toNameEl, footerName, finalNameEl,
  ...
  finaleBtn, muteBtn,
  finalScreen, closeBtn,
  canvas, ctx
];

if (required.some(x => !x)) {
  console.error("Some required DOM nodes are missing. Check index.html IDs/classes.");
} else {
  init();
}
...
function init(){
  ...
  finalNameEl.textContent = DISPLAY_NAME;
  ...
  // close on Esc
  window.addEventListener("keydown", (e) => {
    if (e.key === "Escape") closeFinalScreen();
  });
